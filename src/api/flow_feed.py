...
from src.decode.tx_decoder import decode_tx, UNISWAP_V2_ROUTER, UNISWAP_V3_ROUTER, WETH
...
def map_regime(vpin: float, vpin_threshold: float = 0.4,
                highvol_threshold: float = 0.7) -> str:
    """`predict/regime.py`'nin VPIN eşiğini sözleşme etiketine çevirir.

    Basit eşik kuralı (plan §1b): yüksek VPIN → `toxic`; çok yüksek VPIN
    (aşırı tek-yönlü/toksik akış, yüksek-varyans rejimiyle örtüşen bölge)
    → `highvol`; aksi halde `normal`. Saf ve test edilebilir — dış durum
    okumaz/yazmaz.
    """
    vpin = vpin or 0.0
...
def _synthetic_tx(kind: str, rng: random.Random) -> PendingTx:
    """`src/ingest/mempool_listener.py::_synthetic_tx` ile aynı desenler,
...
class FlowFeed:
    ...
    def __init__(self, mode: str = "simulation", seed: int | None = None,
        ...
        self.flow = RollingFlow(window_sec=window_sec)
