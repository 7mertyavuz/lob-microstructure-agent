...
class ManualClock:
    """Enjekte edilebilir deterministik saat.

    `docs/00-ORTAK-SOZLESME.md` "Belirlilik kuralı": simülatör replay/test
    edebilsin diye `time.time()` yerine enjekte edilebilir saat kullanılır.
    Aynı seed + aynı emir dizisi → aynı `FlowState` dizisi.

    Duvar saati kullanıldığında bu garanti TUTMAZ: aynı emirler farklı
    zamanlamayla verildiğinde pencere farklı boşalır ve imbalance değişir.
    """
    ...
    def tick(self, seconds: float | None = None) -> float:
        """Saati ilerlet ve yeni değeri döndür."""
        self._t += self.step if seconds is None else float(seconds)
        return self._t
...
class RollingFlow:
    ...
    def __init__(self, window_sec: float = 60.0, fast_frac: float = 0.25,
        ...
        # Enjekte edilebilir saat (sözleşmenin belirlilik kuralı). Varsayılan
        # duvar saati canlı mod için doğrudur; simülasyon/test `ManualClock`
        # geçirerek replay garantisini alır.
    ...
    def add(self, sig: ActorSignal) -> None:
        ...
        self._buf[token].append(
            (self._clock(), signed, sig.side, sig.label, sig.est_value_usd)
        )
